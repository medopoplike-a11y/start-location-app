            ...
            {filteredOrders.map((order, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="mb-6 bg-slate-50/30 dark:bg-slate-800/30 p-3 rounded-[24px] border border-slate-100/50 dark:border-slate-700/50 flex flex-col gap-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5">
                        <FileText className="w-3 h-3 text-slate-400 dark:text-slate-500" />
                        <span className="text-[9px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-tight">الفواتير</span>
                      </div>
                    </div>
                    
                    {!order.invoiceUrl && (!order.customers || !order.customers.some(c => c.invoice_url)) && (
                      <div className="flex items-center justify-between py-1 px-2 border border-dashed border-slate-200 dark:border-slate-700 rounded-xl bg-white/50 dark:bg-slate-900/50">
                        <p className="text-[8px] font-bold text-slate-400 dark:text-slate-600 italic">لا توجد فاتورة مرفوعة</p>
