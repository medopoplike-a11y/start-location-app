...
const LoginPage = () => {
  ...
  const apkUrlV = `${FALLBACK_APK_URL.replace('start-location.apk', `start-location-v13.1.1.apk`)}`;
          ...
          <div className="space-y-4 text-sm font-mono text-left" dir="ltr">
            ...
            <div className="p-4 bg-slate-800 rounded-lg border border-slate-700">
              ...
              <p className={ (window as any).Capacitor?.isNativePlatform?.() ? "text-green-400" : "text-yellow-400" }>
                { (window as any).Capacitor?.isNativePlatform?.() ? "NATIVE DIRECT (V13.1.1)" : "WEB STANDARD" }
              </p>
            </div>
