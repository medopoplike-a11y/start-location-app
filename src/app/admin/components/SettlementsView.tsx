  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-black text-slate-900">طلبات التسوية</h2>
          <p className="text-xs text-slate-400 font-bold mt-0.5">{rich.length} طلب معلق</p>
        </div>
        <div className="flex items-center gap-2 bg-amber-50 border border-amber-100 px-4 py-2 rounded-2xl">
          <Clock className="w-4 h-4 text-amber-500" />
          <span className="text-xs font-black text-amber-600">{rich.length} بانتظار المراجعة</span>
        </div>
      </div>
      ...
      {rich.length === 0 ? (
        <div className="bg-white border border-gray-100 rounded-[32px] p-16 text-center">
          ...
          <p className="text-slate-400 font-bold">لا توجد تسويات معلقة</p>
          <p className="text-slate-300 text-xs mt-1">ستظهر هنا طلبات التسوية من المناديب</p>
        </div>
      ) : (
