...
-- إدراج البيانات الافتراضية إذا لم تكن موجودة (V17.3.8)
INSERT INTO public.app_config (id, latest_version, min_version, download_url, driver_commission, vendor_commission)
VALUES (1, 'V17.3.8', '0.1.0', 'https://github.com/medopoplike/start-location-app/releases', 15, 20)
ON CONFLICT (id) DO UPDATE SET 
...
SET latest_version = 'V17.3.8', updated_at = NOW() 
WHERE id = 1;
