...
-- إدراج البيانات الافتراضية إذا لم تكن موجودة (V17.0.0)
INSERT INTO public.app_config (id, latest_version, min_version, download_url, driver_commission, vendor_commission)
VALUES (1, 'V17.0.0', '0.1.0', 'https://github.com/medopoplike/start-location-app/releases', 15, 20)
ON CONFLICT (id) DO UPDATE SET 
...
SET latest_version = 'V17.0.0', updated_at = NOW() 
WHERE id = 1;
