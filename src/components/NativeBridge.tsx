...
export const NativeBridge = () => {
  ...
  useEffect(() => {
    ...
    const setupNative = async () => {
      // V17.3.2: HARD RESET GUARD - Clear stale sessions on version upgrade
      ...
      try {
        ...
        const CURRENT_VERSION = "17.3.2";

        if (lastBootVersion !== CURRENT_VERSION) {
          console.log(`NativeBridge: [V17.3.2] New version detected (${lastBootVersion} -> ${CURRENT_VERSION}). Performing safety cleanup...`);
      ...
      const performOtaCheck = async () => {
        try {
          ...
          console.log(`NativeBridge: [V17.3.2] Checking for OTA updates...`);
          // V17.3.2: TEMPORARY DISABLE AUTO-RELOAD TO BREAK LOOPS
