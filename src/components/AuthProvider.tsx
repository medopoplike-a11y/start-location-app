...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    ...
    // V17.7.1: Industrial Stability
    const initAuth = async () => {
      try {
        console.log("[AuthV17.7.1] Standard session check...");
        const { data: { session } } = await supabase.auth.getSession();
        
        if (session) {
          await updateState(session, "init");
        } else {
          console.log("[AuthV17.7.1] No session found, setting loading false");
          loadingRef.current = false;
          setLoading(false);
        }
      } catch (e) {
        console.error("[AuthV17.7.1] Init error", e);
        if (active) {
            loadingRef.current = false;
            setLoading(false);
        }
    ...
    // V17.7.1: Emergency Splash Timeout
    const splashTimeout = setTimeout(() => {
      if (loadingRef.current && active) {
        console.warn("[AuthV17.7.1] Splash timeout reached. Forcing loader release.");
        loadingRef.current = false;
        setLoading(false);
      }
    }, 10000);
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      console.log(`[AuthV17.7.1] Auth event: ${event}`);
      if (!active) return;
